...
public class SoapEngine extends AbstractStepEngine {
    ...
    private void handleAssertion(TestCase tc, TestStep step, ExecutionContext context, String yamlFile, int stepIndex, Boolean printExecution,
        ...
        if (expected != null) {
            YamlPlaceholderResolver resolver = new YamlPlaceholderResolver();
            String resolvedStr = resolver.resolve(tc, expected.toString(), tc.getVariables(), yamlFile, stepIndex, "expected");
        ...
        case "HEADERS":
            ...
            String actualKey = headers.keySet().stream().filter(k -> k.equalsIgnoreCase(finalPath)).findFirst()
                    .orElse(null);
        ...
        if (Boolean.TRUE.equals(printExecution) && isVerbose) {
            ...
            logAssertionResult(logger, "SOAP", passed, action, expectedStr, actual, passed ? null : res.getBody());
        }

        if (!passed) {
            throw new RuntimeException(
                    String.format("Assertion %s failed! Expected: %s, Actual: %s", action, expected, actual));
        }
