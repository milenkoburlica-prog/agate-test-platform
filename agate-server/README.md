...
```cmd
startTests.bat [USER] [INSTANCE] [APP] [TEST_SUITE] [TEST_CASE] [PRIORITY]
```
...
```cmd
startTests.bat Tester1 DEMOS demo cmd_engine_demo
```
...
```cmd
startTests.bat Tester1 DEMOS demo cmd_engine_demo "TC Hello World 1"
```
...
```cmd
startTests.bat Milenko DEMOS demo cmd_engine_demo "" HIGH
```
...
reports/
 └── ECS_SYST_AUT1/
      └── demo/
           └── cmd_engine_demo/
                ├── Latest_Report.html
                └── SuiteReport_20260621_151756.html
```
...
```properties
ECS_SYST_AUT1.Milenko.cardNumber=12345678
```
...
```cmd
startTests.bat Milenko ECS_SYST_AUT1 MyFirstProject hello_world_02
```
